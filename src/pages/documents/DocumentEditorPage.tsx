...
const DocumentEditorPage = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try { JSON.parse(formData.technicalSpecsJson); }
        catch (e) { return toast.error("Cú pháp JSON không hợp lệ"); }
        ...
        <Layout>
            <div className="max-w-7xl mx-auto p-6 space-y-6">
                <header className="flex justify-between items-center">
                    <h1 className="text-2xl font-bold text-white italic">Biên tập: <span className="text-primary-400 not-italic">{doc.title}</span></h1>
                    ...
                    <form onSubmit={handleSubmit} className="lg:col-span-3 space-y-6">
                        <Card className="p-6 space-y-4 border-l-4 border-primary-500 bg-dark-900/40">
                            ...
                            <div className="space-y-2">
                                ...
                                <textarea
                                    className="w-full bg-dark-800 border border-dark-700 rounded-lg p-3 text-white text-sm focus:ring-1 focus:ring-primary-500 outline-none min-h-[100px] transition-all"
                            ...
                            <div className="space-y-2">
                                <label className="block text-xs font-medium text-accent-green mb-2 uppercase tracking-widest font-bold">Thông số kỹ thuật (JSON)</label>
                                <textarea
                                    className="w-full bg-dark-950 border border-dark-700 rounded-lg p-4 text-accent-green font-mono text-sm focus:border-accent-green outline-none min-h-[350px] shadow-inner"
                                    value={formData.technicalSpecsJson}
                                    onChange={(e) => setFormData(p => p ? ({ ...p, technicalSpecsJson: e.target.value }) : null)}
                                />
                            </div>
                        </Card>
...
const MetaRow = ({ label, value, badgeClass }: { label: string; value: string; badgeClass?: string }) => (
    <div className="flex justify-between items-center text-[11px] py-0.5">
        ...
        <span className={badgeClass ? `px-2 py-0.5 rounded border ${badgeClass} font-bold` : "text-gray-300 font-medium"}>
            {value}
        </span>
    </div>
);
...
const LoadingSpinner = () => (
    <div className="h-screen flex items-center justify-center bg-dark-950">
        <div className="animate-spin rounded-full h-10 w-10 border-t-2 border-primary-500 shadow-[0_0_15px_rgba(59,130,246,0.5)]"></div>
    </div>
);
