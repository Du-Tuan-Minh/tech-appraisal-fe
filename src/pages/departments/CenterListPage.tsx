...
const CenterListPage = () => {
    ...
    const fetchDepartments = useCallback(async (page: number = 1) => {
        ...
        try {
            ...
            toast.error(err.response?.data?.message || "Không thể tải danh sách trung tâm.");
        } finally {
            setIsLoading(false);
        }
    }, [pagination.limit]);
        ...
        <Layout>
            <div className="max-w-7xl mx-auto p-6 space-y-8">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                    <div>
                        <h1 className="text-3xl font-bold text-white italic tracking-tight">Danh Sách Trung Tâm</h1>
                        <p className="text-primary-400 mt-1 italic text-sm">Quản lý các đơn vị cấp cao và cấu trúc tổ chức tổng thể</p>
                    </div>
                    <div className="flex gap-3">
                        <Button variant="ghost" onClick={() => setIsJoinOpen(true)}>Tham gia bằng mã</Button>
                        <Button variant="primary" onClick={() => setIsCreateOpen(true)}>+ Tạo Trung Tâm</Button>
                    </div>
                </div>
