...
const ManagerDashboardPage = () => {
    ...
    const [activeTab] = useState<string>("progress");
    ...
    useEffect(() => {
        ...
        const fetchDashboardData = async () => {
            ...
            try {
                ...
                const summaryPromise = currentMode === "personal"
                    ? dashboardService.getManagerSummary()
                    : dashboardService.getManagerRequestedDocumentSummary();

                const overduePromise = currentMode === "personal"
                    ? documentService.getOverdueDocuments(overdueFilters)
                    : documentService.getManagerRequestedOverdueDocuments(overdueFilters);
                ...
                if (!isMounted || viewMode !== currentMode) return;
                ...
                console.error(`[Dashboard Critical Error] Mode: ${currentMode}`, err);
                const errorMsg = err.response?.data?.message || 'Đã có lỗi xảy ra khi tải dữ liệu bảng điều khiển. Vui lòng thử lại sau.';
                toast.error(`${currentMode === "department" ? "[Phòng ban] " : ""}${errorMsg}`);
            } finally {
                if (isMounted && viewMode === currentMode) {
                    setIsLoading(false);
                }
            }
        };
    ...
    const formatOverdueDynamic = (deadline: string | null | undefined): string => {
        ...
        const remainingHours = Math.abs(diffMs) / (1000 * 60 * 60);
    ...
    const handleStatCardClick = (type: ManagerDashboardDocumentType | DepartmentDocumentStatusType) => {
        navigate(`/manager/dashboard/documents?type=${type}&mode=${viewMode}&page=1`);
    };

    const cardConfigs = viewMode === "personal"
        ? {
            card1: { title: "TÀI LIỆU ĐANG XỬ LÝ NỘI BỘ", count: summary?.internalSigningCount || 0, type: ManagerDashboardDocumentType.InternalSigning },
            card2: { title: "ĐÁNH GIÁ CHỜ XỬ LÝ", count: summary?.pendingReviewCount || 0, type: ManagerDashboardDocumentType.PendingReview },
            card3: { title: "CẦN XÁC NHẬN", count: summary?.needConfirmationCount || 0, type: ManagerDashboardDocumentType.NeedConfirmation },
            card4: { title: "TÀI LIỆU BỊ TỪ CHỐI", count: summary?.rejectedCount || 0, type: ManagerDashboardDocumentType.Rejected },
        }
        : {
            card1: { title: "DANH MỤC ĐANG ĐÁNH GIÁ", count: deptSummary?.reviewingDocuments || 0, type: DepartmentDocumentStatusType.Reviewing },
            card2: { title: "ĐÁNH GIÁ PHÒNG BAN QUÁ HẠN", count: deptSummary?.overdueReviewDocuments || 0, type: DepartmentDocumentStatusType.OverdueReview },
            card3: { title: "TÀI LIỆU ĐÃ PHÁT HÀNH", count: deptSummary?.issuedDocuments || 0, type: DepartmentDocumentStatusType.Issued },
            card4: { title: "TÀI LIỆU PHÒNG BAN BỊ TỪ CHỐI", count: deptSummary?.rejectedDocuments || 0, type: DepartmentDocumentStatusType.Rejected },
        };
        ...
        <Layout>
            <div className="max-w-7xl mx-auto p-6 space-y-8 bg-[#090d16] min-h-screen text-gray-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <StatCard
                        title={cardConfigs.card1.title}
                        count={cardConfigs.card1.count}
                        icon={<span>📄</span>}
                        isActive={activeTab === "progress"}
                        activeBorderColor="border-blue-500"
                        onClick={() => handleStatCardClick(cardConfigs.card1.type)}
                    />
                    <StatCard
                        title={cardConfigs.card2.title}
                        count={cardConfigs.card2.count}
                        icon={<span>⏱️</span>}
                        isActive={activeTab === "review"}
                        activeBorderColor="border-yellow-500"
                        onClick={() => handleStatCardClick(cardConfigs.card2.type)}
                    />
                    <StatCard
                        title={cardConfigs.card3.title}
                        count={cardConfigs.card3.count}
                        icon={<span>✅</span>}
                        isActive={activeTab === "confirm"}
                        activeBorderColor="border-green-500"
                        onClick={() => handleStatCardClick(cardConfigs.card3.type)}
                    />
                    <StatCard
                        title={cardConfigs.card4.title}
                        count={cardConfigs.card4.count}
                        icon={<span>⚠️</span>}
                        isActive={activeTab === "rejected"}
                        activeBorderColor="border-red-500"
                        onClick={() => handleStatCardClick(cardConfigs.card4.type)}
                    />
                </div>
